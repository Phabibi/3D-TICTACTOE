...
exports.numberToIndex =  function numberToIndex(number){
  ...
  function check2Dboard(board){
    var ret = -1;
    for(var i = 0; i< 3; i++){
      (checkRow(board[i]) != -1)? ret = checkRow(board[i]) : console.log("no winner in row " + i);
      (checkColumn(board, i) != -1)? ret = checkColumn(board,i): console.log("no winner in col "+ i);
    }
  ...
  function checkDiagonals(board){
    var ret = -1;
    if(board[0][2] != -1 && board[2][0] != -1 && board[1][1] != -1){
      var sum = board[0][2] + board[1][1] + board[2][0];
      (sum == 3 || sum == 0)? ret = board[1][1]: ret = -1;
      return ret;
    }
    if(board[0][0] != -1 && board[2][2] != -1 && board[1][1] != -1){
